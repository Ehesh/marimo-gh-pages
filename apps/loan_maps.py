...
def _(
    ...
    m2 = create_map_from_data(map2, icon_map2, tooltip_map2)
    m1 = create_map_from_data(map1, icon_map1, tooltip_map1)
    return m1, m2
...
def _():
    ...
    return defaultdict, os, svg_to_data_url


@app.cell
def _(defaultdict, ol, os, svg_to_data_url):
    # All your other imports and the svg_to_data_url function remain the same.

    def create_map_from_data(geojson_data_dict, icon_folder, tooltip_template="<strong>Details</strong><br>{{details}}"):
        """
        Creates a map from GeoJSON data, an icon folder, and a tooltip template.

        Args:
            geojson_data_dict (dict): The Python dictionary of your GeoJSON data.
            icon_folder (str): The file path to the folder of your .svg icons.
            tooltip_template (str): An HTML/Mustache string for the tooltip format.

        Returns:
            An OpenLayers map widget or None if an error occurs.
        """
        # ... (all the logic for grouping features and creating layers remains exactly the same)
        ...
        try:
            for filename in os.listdir(icon_folder):
                if filename.endswith(".svg"):
                    ...
                    available_icons[filename] = svg_to_data_url(icon_path)
        except FileNotFoundError:
            print(f"Error: The icon folder was not found at '{icon_folder}'")
            return None
        if not available_icons:
            print(f"Error: No .svg files were found in the folder '{icon_folder}'")
            return None
        for feature in geojson_data_dict.get('features', []):
            ...
            if icon_filename and icon_filename in available_icons:
                icon_data_url = available_icons[icon_filename]
                grouped_features[icon_data_url].append(feature)
            else:
                print(f"Warning: Skipping feature. Icon '{icon_filename}' not found or not specified. Feature properties: {properties}")
        ...
        # The function now uses the template that was passed into it.
...
def _():
    Problem_statement = f"""
    ## Problem statement
    The study of language contact and lexical transfer between languages is a cornerstone for understanding the historical, social, and cultural dynamics that have shaped human communication. **Loanwords**, in particular, serve as empirical and quantifiable indicators of influence between different speech communities. In the digital age, the availability of large linguistic databases has opened up new avenues for analyzing these phenomena on an unprecedented global scale.
    ...
    Methodology = f"""
    ## Methodology
    Your provided markdown code for the Methodology section contains a number of footnotes that are not correctly referenced within the body of the text. To fix this, I need to match the footnotes with the appropriate words in the text and ensure they are assigned correctly.

    Here is the corrected markdown code with the footnotes properly assigned:
    ...
    [^Matplotlib]: A comprehensive library for creating static, animated, and interactive visualizations in Python. It provides a vast array of plotting options, from simple line plots to complex 3D graphs, and is a cornerstone of the scientific computing ecosystem in Python.
    ...
    [^Glotolog]: A tool or project that appears to be related to the organization and visualization of linguistic data, specifically concerning language and dialect relationships. Based on available information, it likely deals with creating visualizations or interactive maps that show connections between languages.
    ...
    **Conclusions and Interpretation:**
    ...
    AN = f"""
    ## Author's note
    This project began with a simple curiosity about the intersection of language and data, rather than a specific research question. My initial idea was very ambitious, and I soon realized I needed to narrow my focus and find a more manageable dataset. The opportunity came during a summer ([Programa Delfín](https://www.programadelfin.org.mx/)) where Dr. Zarina Estrada, in a talk on typology and linguistic diversity, mentioned the World Loanword Database (WOLD). That resource became the perfect starting point for my exploration. I joined forces with Erubiel Aceves for project brainstorming and splitting the workload, he focused more on the linguistic research side, while I handled data curation.
    ...
    During this whole process, I’ve been going through a very tough time with mental health, so I want to take a moment to give special thanks to Dr. Zarina, and to my friends Erubiel A. and Angela V. for their support.
